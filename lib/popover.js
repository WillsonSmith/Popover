	...
	var Popover = function () {
	  function Popover(node, focusableFunction) {
	    ...
	    this.node.addEventListener('keydown', function (evt) {
	      evt.preventDefault();
	      var popoverFocusable = _this.focusableElements(_this.node);
	      var focusableElements = _this.focusableElements().filter(function (element) {
	      ...
	      if (evt.keyCode === 9 && !_this.shiftDown) {
	        ...
	        if (focusedIndex === popoverFocusable.length - 1) {
	          nextTabElement(focusableElements.indexOf(_this.activator), focusableElements).focus();
	          _this.deactivatePopover();
	        }
	      }
